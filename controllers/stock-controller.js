...
export const getOpenStock = async (req, res) => {
  try {
    ...
    await Stock.deleteMany({
      _id: { $ne: existingOpenStock[0]._id },
      date: currentDate,
      status: "open-stock",
      tyreSize: openStockRecords[0].tyreSize,
      location: openStockRecords[0].location,
...
export const deleteDuplicateRecords = async () => {
  try {
    ...
    // Delete duplicate records based on unique combinations
    for (const combination of uniqueCombinations) {
      const recordsToDelete = await Stock.find({
        tyreSize: combination.tyreSize,
      ...
      for (const record of recordsToDelete) {
        await Stock.findByIdAndDelete(record._id);
      }
    }

    console.log("Duplicate records deleted successfully");
  } catch (err) {
    console.error("Failed to delete duplicate records", err);
  }
};

// Call the function to delete duplicate records
